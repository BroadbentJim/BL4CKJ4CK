...
def game(loop, strategy, wanted):
    ...
    total_player_wins = player_wins + split_player_wins
    total_dealer_wins = dealer_wins + split_dealer_wins
    net_gain = total_player_wins - total_dealer_wins
    ...
    # print("This means the player had a " + str(round(total_player_wins / total_games * 100, 2)) + "% win rate.")
    # print("This also means that the player's gain percentage was " + str(round(100*net_gain/total_dealer_wins, 2)) + "%")
    ...
    #return Resultspos, Results

    # Position = int(input("What player hand score would you like to see?")) - 4
    # plt.ylabel('Probability to hit')
    # plt.xlabel('Score of dealer hand')
    # plt.title('Given player score of ' + str(Position + 4))
    # width = 0.35


    #plt.bar(Dealer, Hit_Ratio_Matrix[Position], width)
    #plt.show()

    # plt.xlabel("Simulations")
    # plt.ylabel("Net Gain")
    # plt.plot(Results_coarse_pos, Results_coarse, color='grey')
    # plt.plot(Resultspos, Results)
    # plt.show()
    return np.array(Resultspos), np.array(Results)
